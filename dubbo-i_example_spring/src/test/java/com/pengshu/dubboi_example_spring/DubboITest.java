...
public class DubboITest {
	...
	public static void beforeClass() throws Exception {
		DubboI_Configuration.newInstance("dubboi.properties"); // 指定dubbo-i配置文件，默认启动restful服务
		//DubboI_Configuration.newInstance("dubboi.properties", false); // 指定dubbo-i配置文件，不启动restful服务
	...
	public void testSayHelloImpl() {
		try {
			...
			RpcGenericService sayHelloService = RpcGenericService.Create("com.pengshu.dubboi_example_spring.service.SayHello", "1.0.0", "", DubboI_Configuration.Loadbalance.random.toString(), 100, 10, 2, 0); // 获取service的同时，指定客户端的负载均衡等配置
	...
	public void testChineseSayHelloImpl() { // 同一个接口的不同实现类，可以用group来区分
		try {
			RpcGenericService sayHelloService = RpcGenericService.Create("com.pengshu.dubboi_example_spring.service.SayHello", "1.0.0", "chinese", DubboI_Configuration.Loadbalance.random.toString(), 100, 10, 2, 0); // 获取service的同时，指定客户端的负载均衡等配置
